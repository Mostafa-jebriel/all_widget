...
class _HomePageState extends State<AllNotifications> {
  ...
  if (notificationAppLaunchDetails?.didNotificationLaunchApp ?? false) {
    ...
    Navigator.of(context).push(MaterialPageRoute(builder: (_) => Test(payload: notificationAppLaunchDetails!.payload)));
  }
          ...
          child: Center(
            child: Column(
              children: <Widget>[
                ...
                _InfoValueString(
                  title: 'Did notification launch app?', value: widget.didNotificationLaunchApp,),
                if (widget.didNotificationLaunchApp)
                  _InfoValueString(title: 'Launch notification payload:', value: widget.notificationAppLaunchDetails!.payload,),
                ...
                if (kIsWeb || !Platform.isLinux) ...<Widget>[
                  ...
                  PaddedElevatedButton(
                    ...
                    'Schedule weekly days '
                        'local time zone',
                    onPressed: () async {
                      await _scheduleWeeklyDayNotification();
                    },
                  ),
                  PaddedElevatedButton(
                    buttonText:
  ...
  Future<void> _repeatNotification() async {
    ...
    await flutterLocalNotificationsPlugin.periodicallyShow(0, 'repeating title',
        'repeating body repeat', RepeatInterval.everyMinute, platformChannelSpecifics,
        androidAllowWhileIdle: true);
  }
  //b7
  ...
  Future<void> _scheduleDailyTenAMLastYearNotification() async {
    await flutterLocalNotificationsPlugin.zonedSchedule(
        0, 'daily scheduled notification title', 'daily scheduled notification body',
  ...
  Future<void> _scheduleWeeklyTenAMNotification() async {
    await flutterLocalNotificationsPlugin.zonedSchedule(
        0, 'weekly scheduled notification title', 'weekly scheduled notification body Weekly',
        _nextInstanceOfTenAM(),
        const NotificationDetails(
  ...
  Future<void> _scheduleWeeklyDayNotification() async {
    await flutterLocalNotificationsPlugin.zonedSchedule(
        0, 'weekly scheduled notification Days', 'weekly scheduled notification body Days',
        _nextInstanceDays(),
        const NotificationDetails(
          android: AndroidNotificationDetails('weekly notification channel id',
              'weekly notification channel name',
  ...
  tz.TZDateTime _nextInstanceDays() {
    final tz.TZDateTime now = tz.TZDateTime.now(tz.local);
    tz.TZDateTime scheduledDate =
    tz.TZDateTime(tz.local, now.year, now.month, now.day, 15,3);
    if (scheduledDate.isBefore(now)) {
      scheduledDate = scheduledDate.add(const Duration(days: 1));
    }
    List<int> days=[
  ...
  Future<void> _scheduleWeeklyMondayTenAMNotification() async {
    await flutterLocalNotificationsPlugin.zonedSchedule(0, 'weekly scheduled notification title',
